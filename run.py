...
uamain = f"Davik/2.1.0 (Linux; U; Android {android}; {str(rc(redmi))}) Build/{build_device}) [FBAN/MessengerLite;FBAV/{versi_chrome};FBPN/com.facebook.mlite;FBLC/{language};FBBV/{versi_app};FBCR/{simcard};FBMF/{merk_device};FBBD/{brand_device};FBDV/{model_device};FBSV/{android};FBCA/{cpu_device};FBDM/"+str(large_device)+";]"
uamain1 = f"Davik/2.1.0 (Linux; U; Android {android}; {str(rc(redmi))}) Build/{build_device}) [FBAN/MessengerLite;FBAV/{versi_chrome};FBPN/com.facebook.mlite;FBLC/{language};FBBV/{versi_app};FBCR/{simcard};FBMF/{merk_device};FBBD/{brand_device};FBDV/{model_device};FBSV/{android};FBCA/{cpu_device};FBDM/"+str(large_device)+";]"
...
def key():
    ...
    try:
        ses = requests.Session()
        asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyI2OTI5NTA4OCIsIjBSeEFWWWt1OE9ucnR5bGJNSzMvTGRaQnBXZTYzUTdCaDNhcThhaDciXQ==&ProductId=23002&Key=%s&Sign=True"%(key)).json()['licenseKey']['key']
...
def cek():
    try:
        ...
        licen()
    try:
        lopi = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyI2OTI5NTA4OCIsIjBSeEFWWWt1OE9ucnR5bGJNSzMvTGRaQnBXZTYzUTdCaDNhcThhaDciXQ==&ProductId=23002&Key=%s"%(x)).json()['licenseKey']['key']
        login()
    except KeyError:
...
def menu(my_name,my_id):
    try:
        ...
        met = ses.get('https://app.cryptolens.io/api/key/Activate?token=WyI2OTI5Mjc0MiIsIk9oLzFWeTlmaytRallHUFBPbVVTdGI4YVJzekRwSE9WelRvSWZmYWwiXQ==&ProductId=23002&Key='+lisen).json()
        men = met['licenseKey']
        key = men['key'][0:11]
