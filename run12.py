				...
				if "session_key" in post.text and "EAA" in post.text:
					...
					if user in self.ok or user in self.cp:
						...
						if "muncul" in self.apk:
							...
							tree = Tree(Panel.fit(f"""{H2}{self.ok}|{pw}{P2}""",style=f"{color_panel}"),guide_style="bold grey100")
					...
					if user in self.ok or user in self.cp:
						...
						self.cp.append(user)
						tree = Tree(Panel.fit(f"""{K2}{self.cp}|{pw}{P2}""",style=f"{color_panel}"),guide_style="bold grey100")
